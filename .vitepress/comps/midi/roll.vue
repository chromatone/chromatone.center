...
function drawVertical() {
  ...
  for (let i = 0; i < 127; i++) {
    ...
    ctx.fillRect(num + 2, 0, 1, state.speed)
  }
  score.notes.forEach(note => {
    ctx.fillStyle = colorIt((note + 3) % 12, 1)
    ctx.fillRect(note * 4, 0, 4, state.speed * 2)
  })
...
function drawHorizontal() {
  ...
  for (let i = 0; i < 127; i++) {
    let num = 127 - i
    ctx.fillStyle = colorIt((num + 3) % 12, 1, 0.1)
    ctx.fillRect(state.width - state.speed, state.height - num * 2, state.speed, 0.5)
  }
  score.notes.forEach(note => {
    ctx.fillStyle = colorIt((note + 3) % 12, 1)
