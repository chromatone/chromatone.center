...
}`,b={class:`relative rounded-lg overflow-hidden pointer-events-auto touch-revert h-screen`,id:`screen`},x={__name:`ChromaPalette`,setup(x){let{tuner:S,init:C}=_(),{width:w,height:T}=m(),E=o(0),D=o(!1),{activeNotes:O}=v(),k=d(()=>y),A=d(()=>{let e=Array(12).fill(0);for(let t in O.value){let n=(Number(t)-9)%12;e[n]+=O.value[t]}return e.map((e,t)=>{let n=S.aChroma[t]**2;return e+(n>S.chromaAvg&&!S.note.silent?n:0)})}),j=p(A,{duration:200,transition:g.easeOutExpo}),M=d(()=>{let e=Array(16).fill(0);return j.value.forEach((t,n)=>e[n]=t),e});function N(e){E.value=(Math.sin(e.iTime)+1)/2}let P=d(()=>w.value||T.value?Math.random():0),F=h(P,100),I=d(()=>`
