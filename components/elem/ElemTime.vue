...
import { useTime } from '#/use/elem/useTime';
import { useElementary } from '#/use/elem/useElementary';

const { render, audio } = useElementary()
const { time, controls, cv, play, pause, stop, init } = useTime()
...
.absolute.top-18.bg-light-200.p-8.z-200.w-full.cursor-pointer.rounded-lg.text-xl.font-bold.border-1.dark-bg-dark-400(
  @click="init()"
  v-if="!audio.initiated") Click to start watching time passing

.p-0  
  p {{ controls }}
button(@click="play()") PLAY
ControlRotary(v-model="controls['time:volume']")
...
//-     //- ElemMetronome.sticky.top-2.z-100.w-full
...
//-         button.text-button(@click="play()")
//-           .i-la-play
//-         button.text-button(@click="pause()")
//-           .i-la-pause
//-         button.text-button(@click="stop()")
//-           .i-la-stop

//-     .text-right Started At:
//-     .p-0.font-bold {{ controls['time:started'].toFixed(2) || 0 }}
//-     .text-left.text-sm seconds

//-     .text-right Paused At:
//-     .p-0.font-bold {{ controls['time:paused'].toFixed(2) || 0 }}
//-     .text-left.text-sm seconds
