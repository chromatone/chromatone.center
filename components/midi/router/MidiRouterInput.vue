...
<template lang="pug">
.p-4.shadow-lg.rounded-2xl.bg-light-900.dark-bg-dark-300.relative.border-2.shadow-lg(style="flex: 1 1 200px")
  .flex.justify-between
    ...
    .flex-col.text-xs.font-mono.flex.gap-2.text-center
      .display(v-if="input.note" :style="{ backgroundColor: color }")
        ...
        .px-1.rounded-sm(:style="{ backgroundColor: color }")
      ...
      .display(v-if="input.cc")
        .indicator(:style="{ transform: `scale(${input?.cc?.value},1)` }")
        i
          .
            CH{{
            input?.cc?.channel
