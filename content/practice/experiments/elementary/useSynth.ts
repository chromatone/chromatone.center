...
const mic = reactive({
  ...
  async open() {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      ...
      let streamSource = audio.ctx.createMediaStreamSource(micStream);
      streamSource.connect(audio.node);
    }
  }
})
...
export function useSynth({
  ...
  const synth = reactive({
    ...
    async init() {
      ...
      audio.core.on('load', async function () {
