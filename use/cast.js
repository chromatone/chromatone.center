...
export function useRecording() {
  ...
  async function toggleAvatar() {
    if (currentCamera.value === "none") return

    if (showAvatar.value) {
      showAvatar.value = false
      if (!recording.value) closeStream(streamCamera)
    } else {
      await startCameraStream()
      if (streamCamera.value) showAvatar.value = !!streamCamera.value
    }
  }
  ...
  async function startCameraStream() {
    ...
    if (!streamCamera.value) {
      if (currentCamera.value === "none" && currentMic.value === "none") return

      streamCamera.value = await navigator.mediaDevices.getUserMedia({
        video:
          currentCamera.value === "none" || recordCamera.value !== true
            ? false
            : {
              deviceId: currentCamera.value
            },
        audio:
          currentMic.value === "none"
            ? false
            : {
  ...
  async function startRecording(customConfig) {
    ...
    streamCapture.value
      .getVideoTracks()
      .forEach(videoTrack => streamSlides.value.addTrack(videoTrack))
    ...
    if (streamCamera.value) {
      ...
      recorderCamera.value = new Recorder(streamCamera.value, config)
      recorderCamera.value.startRecording()
    }

    recorderSlides.value = new Recorder(streamSlides.value, config)
  ...
  async function stopRecording() {
    ...
    recorderCamera.value?.stopRecording(() => {
      ...
      if (!showAvatar.value) closeStream(streamCamera)
    })
  ...
  useEventListener("beforeunload", event => {
    ...
    if (confirm("Recording is not saved yet, do you want to leave?")) return
