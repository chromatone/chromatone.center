...
import { midi, playNote } from "./midi"
...
export function setupKeyboard() {
  ...
  document.addEventListener('keydown', e => {
    ...
    playNote(noteKeys[e.code].note + (noteKeys[e.code].offset + 4 + midi.offset), 1, undefined, false)
  })

  document.addEventListener('keyup', e => {
    if (!noteKeys[e.code]) return
    playNote(noteKeys[e.code].note + (noteKeys[e.code].offset + 4 + midi.offset), 0, undefined, false)
  })
}
